...
const fc_proyectado = {
    guardar: async function ({ detalles, ...objeto }, idUsuario) {
        try {
            ...
            const cabeceraBd = await mssqlConcar.ejecutarQueryPreparado(`select id from fc_cabecera_flujo_real where mes='${objeto.mes}' and anio='${objeto.anio}' and idEstado=1`, {}, true)
            if (cabeceraBd) {
                idCabecera = cabeceraBd.id
                await mssqlConcar.ejecutarQueryPreparado(`update fc_cabecera_flujo_real set anio='${objeto.anio}', mes='${objeto.mes}',montoSoles=${montoSoles},montoDolares=${montoDolares} where id=${objeto.id}`, {})
/*                 await mssqlConcar.actualizar("fc_cabecera_flujo_real", { anio: objeto.anio, mes: objeto.mes, montoSoles, montoDolares }, { id: idCabecera })
 */            } else {
                const query = await mssqlConcar.queryInsertarEnLotes("fc_cabecera_flujo_real", [{ anio: objeto.anio, mes: objeto.mes, montoSoles, montoDolares, fechaRegistro, idUsuario, idEstado: 1, fechaFlujo: fechaRegistro }])
                const cabecera = await mssqlConcar.ejecutarQueryPreparado(`${query};SELECT SCOPE_IDENTITY() as id;`, {}, true)
                idCabecera = cabecera.id
            }
            await mssqlConcar.ejecutarQueryPreparado(`delete from fc_detalle_flujo_real where idCabecera=${idCabecera}`, {})
            if (detalles.length > 0) {
                const detallesMap = detalles.map(d => ({
                    ...
                    numeroDocumento: d.numeroDocumento ? d.numeroDocumento.toString() : null,
                    ...
                    dsecue: d.dsecue ? d.dsecue : null,
                    nroDiario: d.nroDiario ? d.nroDiario : null,
                    nroComprob: d.nroComprob ? d.nroComprob : null,
                    fechaComprob: d.fechaComprob ? moment(d.fechaComprob, ['DD/MM/YYYY', 'YYYY-MM-DD']).format("YYYY-MM-DD").toString() : null

                }))
                await mssqlConcar.insertar("fc_detalle_flujo_real", detallesMap)
            }
        } catch (error) {
    ...
    actualizarTipoServicioFamiliaYSubFamilia: async function ({ tipoServicio, familia, subFamilia, tipoFlujo, id, periodo }) {
        ...
        await mssqlConcar.actualizar('fc_detalle_flujo_real', { idFamilia: familia.id, idSubFamilia: subFamilia.id, idTipoServicio: tipoServicio.id, idTipoFlujo: tipoFlujo.id }, { id })
    },
    importarConcar: async function ({ id, anio, mes }) {
        try {
            await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_FLUJOREAL_INSERTADESDECONCAR  '${anio}', '${mes}',${id}`, {})
            const detalleNuevo = this.listarDetallePorCabecera(id)
            return detalleNuevo
        } catch (error) {
            console.error(error)
            throw error
    ...
    exportarExcel: async function (id) {
        ...
        try {
            ...
            const cabecera = await mssqlConcar.ejecutarQueryPreparado(`select * from fc_cabecera_flujo_real where id=${id}`, {}, true)
            const listaDetalles = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_FLUJOREAL_LISTARREGISTRO  '${cabecera.anio}', '${cabecera.mes}', ${id};`, {})
            const listaDetallesResumido = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_FLUJOREAL_LISTARresumen '${cabecera.anio}', '${cabecera.mes}', ${id};`, {})
            const rutaTemplatePedidos = "./template/Exportacion Flujo Real.xlsx";
            if (fs.existsSync(`.${rutaTemplatePedidos}`)) {
                fs.unlinkSync(`.${rutaTemplatePedidos}`);
            }

            await workbook.xlsx.readFile("./template/Plantilla Exportacion Flujo Real.xlsx")
            ...
            return { templateUrl: `/supergen-be/template/Exportacion Flujo Real.xlsx` }
        } catch (error) {
    ...
    listar: async function ({ anio, mes }) {
        try {
            const lista = await mssqlConcar.ejecutarQueryPreparado(`select * from fc_cabecera_flujo_real where idEstado=1 ${anio ? ` and anio='${anio}'` : ''} ${mes ? ` and mes='${mes}'` : ''}`, {})
            return lista
        } catch (error) {
            throw new Error(error)
        }
    ...
    importar: async function (excelRuta) {
        ...
        for (const dataBd of data) {
            ...
            dataProcess.push({ familia, subFamilia, tipoServicio, tipoAnexo, tipoDocumento, tipoFlujo, proveedor: { ruc: dataBd.rucProveedor.toString(), nombre: dataBd.proveedorNombre }, fechaPago: moment(dataBd.fechaPago).format("YYYY-MM-DD"), ...dataBd, moneda: monedaCambio[dataBd.moneda], origen: "I" })
    ...
    listarDetallePorCabecera: async function (id) {
        try {
            const detalles = await mssqlConcar.ejecutarQueryPreparado(`select tipoAnexo.anexo as tipoAnexoNombre,detalleReal.*,familia.descripcion as familiaDescripcion,subFamilia.descripcion as subFamiliaDescripcion,
            tipoServicio.descripcion as tipoServicioDescripcion,tdoc.descripcion as tipoDocumentoDescripcion,tipoFlujo.descripcion as tipoFlujoDescripcion from fc_detalle_flujo_real detalleReal join fc_familia familia on familia.id=detalleReal.idFamilia join fc_subFamilia subFamilia on subFamilia.id=detalleReal.idSubFamilia
            join fc_tipoServicio tipoServicio on tipoServicio.id=detalleReal.idTipoServicio join fc_tdoc tdoc on  tdoc.id=detalleReal.idTipoDocumento join fc_tipoFlujo tipoFlujo on tipoFlujo.id=detalleReal.idTipoFlujo
            join fc_tipo_anexo tipoAnexo on tipoAnexo.id=detalleReal.idTipoAnexo
            where idCabecera=${id}`, {})
    ...
    eliminar: async function (objetoId) {
        try {
            await mssqlConcar.actualizar("fc_cabecera_flujo_real", { activo: 0 }, { id: objetoId })
        } catch (error) {
            throw new Error(error)
        }
