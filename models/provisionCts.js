...
const provisionCts = {
    guardar: async function ({ detalleDocumentos = [], detalles = [], ...cabecera }, usuarioRegistro) {
        try {
            ...
            const cabeceraEntity = await mssqlConcar.ejecutarQueryPreparado(`select id from prov_cabecera where periodo='${periodo}' and tipoPlanilla='${cabecera.tipoPlanilla}' and tipoProvision='CTS' and condicion='${cabecera.condicion}'`, {}, true)
            if (cabeceraEntity) {
                idCabecera = cabeceraEntity.id
            }
            if (!idCabecera) {
                const cabeceraEntidad = await mssqlConcar.ejecutarQueryPreparado(`insert into prov_cabecera(periodo,tipoProvision,tipoPlanilla,condicion,fechaRegistro,idEstado,idUsuario,anio,mes,fecha) values('${periodo}','CTS','${cabecera.tipoPlanilla}','${cabecera.condicion}','${moment().format("YYYY-MM-DD")}',0,${usuarioRegistro},'${cabecera.anio}','${cabecera.mes}','${moment(periodo, 'YYYYMM').startOf("M").format("YYYY-MM-DD")}');SELECT SCOPE_IDENTITY() as id;`, {}, true)
 ...
 */                await mssqlConcar.ejecutarQueryPreparado(query, {})
            ...
            for (const detalle of detallesMap) {
                await mssqlConcar.ejecutarQueryPreparado(`exec SP_INSERTA_DETALLECTS '${detalle.periodo}', '${detalle.tipoPlanilla}', '${detalle.condicion}','${detalle.nroDiario}','${detalle.comprobante}',${idCabecera}`, {})
            }
    ...
    exportarDetalle: async function (idCabecera) {
        try {
            ...
            if (fs.existsSync(`./template/Provision CTS.xlsx`)) {
                fs.unlinkSync(`./template/Provision CTS.xlsx`)
            }
            const borderStyles = {
                top: { style: "thin" },
                left: { style: "thin" },
            ...
            await wor.xlsx.writeFile(`./template/Provision CTS.xlsx`)
            const json = {
                success: true,
                message: "Exportación realizada correctamente.",
                rutaCM: "/supergen-be/template/Provision CTS.xlsx"
            }
            return json;
        } catch (error) {
    ...
    resumen: async function ({ anio, tipoPlanilla }) {
        try {
            const listaPropiedadesDatos = ["SALDO_INICIAL", "ENERO", "FEBRERO", "MARZO", "ABRIL", "PAGOS_MAYO", "AJUSTEPAGOS_MAYO", "MAYO", "JUNIO", "JULIO", "AGOSTO", "SETIEMBRE", "OCTUBRE", "PAGOS_NOVIEMBRE", "AJUSTEPAGOS_NOVIEMBRE", "NOVIEMBRE", "DICIEMBRE", "TOTAL", "ABONOS", "SALDO_ANIO"]
            const dataProcess = []
            const data = await mssqlConcar.ejecutarQueryPreparado(`exec SP_RESUMENCTS '${anio}' , '${tipoPlanilla}'`, {})
    ...
    eliminarProvision: async function ({ anio, mes, tipoPlanilla, condicion }) {
        try {
            ...
            await mssqlConcar.ejecutarQueryPreparado(`delete from prov_cabecera where periodo='${periodo}' and tipoPlanilla='${tipoPlanilla}' and condicion='${condicion}' and tipoProvision='CTS'`, {})
        } catch (error) {
            throw Error(error)
        }
    ...
    listarPago: async function ({ periodo, tipoPlanilla, condicion }) {
        ...
        const data = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_PAGOS '${periodo}','${tipoPlanilla}','${condicion}'`, {})
    ...
    listarPagoDetalles: async function (detalles = []) {
        let dataProcess = []
        for (const detalle of detalles) {
            const data = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_DETALLEPAGOCTS '${detalle.periodo}','${detalle.tipoPlanilla}','${detalle.condicion}','${detalle.nroDiario}','${detalle.comprobante}'`, {})
    ...
    pagosDeResumen: async function ({ anio, tipoPlanilla, nroDni }) {
        const data = await mssqlConcar.ejecutarQueryPreparado(`exec SP_LEEDETALLEPAGOTRABAJADOR '${anio}','${tipoPlanilla}','CTS','${nroDni}'`, {})
    ...
    exportatPagosDeResumen: async function ({ anio, tipoPlanilla, nroDni }) {
        ...
        try {
            ...
            if (fs.existsSync(`./template/Detalle_Pagos_trabajador_cts.xlsx`)) {
                fs.unlinkSync(`./template/Detalle_Pagos_trabajador_cts.xlsx`)
            }
            const borderStyles = {
                top: { style: "thin" },
                left: { style: "thin" },
            ...
            await wor.xlsx.writeFile(`./template/Detalle_Pagos_trabajador_cts.xlsx`)
            const json = {
                success: true,
                message: "Exportación realizada correctamente.",
                rutaCM: "/supergen-be/template/Detalle_Pagos_trabajador_cts.xlsx"
            }
            return json;
        } catch (error) {
            console.log("err", error)
    ...
    exportarResumido: async function ({ anio, tipoPlanilla }) {
        ...
        try {
            const rutaTemplateHC = `./template/Plantilla Resumen Provision CTS Anual.xlsx`;
            if (fs.existsSync(`./template/Resumen Provision CTS.xlsx`)) {
                fs.unlinkSync(`./template/Resumen Provision CTS.xlsx`)
            }
            const borderStyles = {
                top: { style: "thin" },
                left: { style: "thin" },
            ...
            await wor.xlsx.writeFile(`./template/Resumen Provision CTS.xlsx`)
            const json = {
                success: true,
                message: "Exportación realizada correctamente.",
                rutaCM: "/supergen-be/template/Resumen Provision CTS.xlsx"
            }
            return json;
        } catch (error) {
            console.log("err", error)
    ...
    listarDetalles: async function (detalles) {
        let dataProcess = []
        for (const detalle of detalles) {
            const data = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_DETALLECTS '${detalle.periodo}','${detalle.tipoPlanilla}','${detalle.condicion}','${detalle.nroDiario}','${detalle.comprobante}'`, {})
            if (data) {
    ...
    exportarExcelAjuste: async function (detallesFront) {
        try {
            const rutaTemplateHC = `./template/Plantilla Ajustes CTS.xlsx`;
            const detalles = detallesFront
            const montoMensualTotal = detalles.reduce((prev, curr) => prev += curr.montoMensual, 0)
            if (fs.existsSync(`./template/Provision Ajuste Cts.xlsx`)) {
                fs.unlinkSync(`./template/Provision Ajuste Cts.xlsx`)
            }
            const borderStyles = {
                top: { style: "thin" },
                left: { style: "thin" },
            ...
            await wor.xlsx.writeFile(`./template/Provision Ajuste Cts.xlsx`)
            const json = {
                success: true,
                message: "Exportación realizada correctamente.",
                rutaCM: "/supergen-be/template/Provision Ajuste Cts.xlsx"
            }
            return json;
        } catch (error) {
            console.log("err", error)
    ...
    guardarAjuste: async function ({ detalleDocumentos = [], detalles = [], ...cabecera }, usuarioRegistro) {
        const periodo = `${cabecera.anio}${cabecera.mes}`
        const fechaRegistro = moment().format("YYYY-MM-DD")
        const cabeceraBd = await mssqlConcar.ejecutarQueryPreparado(`select id from prov_cabecera where periodo='${periodo}' and tipoPlanilla='${cabecera.tipoPlanilla}' and tipoProvision='CTS' and condicion='${cabecera.condicion}'`, {}, true)
        if (!cabeceraBd) {
    ...
    listarAjustes: async function (detalles = []) {
        let dataProcess = []
        for (const detalle of detalles) {
            const data = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_DETALLEAJUSTECTS  '${detalle.periodo}','${detalle.tipoPlanilla}','${detalle.condicion}','${detalle.nroDiario}','${detalle.comprobante}'`, {})
            dataProcess = dataProcess.concat(data.map(d => ({ nroDni: d.NRO_DNI.trim(), nombres: d.NOMBRES, comprobante: detalle.comprobante, nroDiario: detalle.nroDiario, fechaCese: d.FEC_CESE, fechaIngreso: d.FEC_ING, centroCosto: d.CENTROCOSTO, area: d.AREA, montoMensual: d.MONTO_MES, pagoMensual: d.PAGO_MES, saldoMensual: d.SALO_MES, fechaPago: d.FECHA_PAGO, cuenta: d.CUENTA })))
        }
        return dataProcess
    ...
    detallesDocumentosAjustes: async function ({ periodo, tipoPlanilla, condicion }) {
        ...
        const dataComprobantes = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_AJUSTECTS  '${periodo}','${tipoPlanilla}','${condicion}'`, {})
        return dataComprobantes.map(a => ({ nroDiario: a.DIARIO.trim(), nroComprobante: a.NROCOMPROB, glosa: a.CGLOSA, fechaComprobante: a.FECHA_COMPROB }))
    },
    detalleAjusteTrabajador: async function ({ anio, tipoPlanilla, nroDni }) {
        const data = await mssqlConcar.ejecutarQueryPreparado(`exec SP_LEEDETALLEAJUSTETRABAJADOR '${anio}','${tipoPlanilla}','CTS','${nroDni.trim()}'`, {})
