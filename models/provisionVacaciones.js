...
const provisionVacaciones = {
    guardar: async function ({ detalleDocumentos = [], detalles = [], ...cabecera }, usuarioRegistro) {
        try {
            ...
            const cabeceraEntity = await mssqlConcar.ejecutarQueryPreparado(`select id from prov_cabecera where periodo='${periodo}' and tipoPlanilla='${cabecera.tipoPlanilla}' and tipoProvision='VACACIONES' and condicion='${cabecera.condicion}'`, {}, true)
            if (cabeceraEntity) {
                idCabecera = cabeceraEntity.id
            }
            if (!idCabecera) {
                /*                 const cabeceraEntidad = await mssqlConcar.insertar("prov_cabecera", [{ periodo: `${cabecera.anio}${cabecera.mes}`, tipoProvision: 'VACACIONES', tipoPlanilla: cabecera.tipoPlanilla, condicion: cabecera.condicion, fechaRegistro: moment().format("YYYY-MM-DD"), idEstado: 0, idUsuario: usuarioRegistro, anio: cabecera.anio, mes: cabecera.mes, fecha: moment(`${cabecera.anio}${cabecera.mes}`, 'YYYYMM').startOf("M").format("YYYY-MM-DD") }])
                                 */
                const cabeceraEntidad = await mssqlConcar.ejecutarQueryPreparado(`insert into prov_cabecera(periodo,tipoProvision,tipoPlanilla,condicion,fechaRegistro,idEstado,idUsuario,anio,mes,fecha) values('${periodo}','VACACIONES','${cabecera.tipoPlanilla}','${cabecera.condicion}','${moment().format("YYYY-MM-DD")}',0,${usuarioRegistro},'${cabecera.anio}','${cabecera.mes}','${moment(periodo, 'YYYYMM').startOf("M").format("YYYY-MM-DD")}');SELECT SCOPE_IDENTITY() as id;`, {}, true)
            ...
            for (const detalle of detallesMap) {
                await mssqlConcar.ejecutarQueryPreparado(`exec SP_INSERTA_DETALLEVACACIONES '${detalle.periodo}', '${detalle.tipoPlanilla}', '${detalle.condicion}','${detalle.nroDiario}','${detalle.comprobante}',${idCabecera}`, {})
            }
    ...
    guardarAjuste: async function ({ detalleDocumentos = [], detalles = [], ...cabecera }, usuarioRegistro) {
        const periodo = `${cabecera.anio}${cabecera.mes}`
        const fechaRegistro = moment().format("YYYY-MM-DD")
        const cabeceraBd = await mssqlConcar.ejecutarQueryPreparado(`select id from prov_cabecera where periodo='${periodo}' and tipoPlanilla='${cabecera.tipoPlanilla}' and tipoProvision='VACACIONES' and condicion='${cabecera.condicion}'`, {}, true)
        if (!cabeceraBd) {
    ...
    exportarExcelAjuste: async function (detallesFront) {
        try {
            const rutaTemplateHC = `./template/Plantilla Ajustes Vacaciones.xlsx`;
            const detalles = detallesFront
            const montoMensualTotal = detalles.reduce((prev, curr) => prev += curr.montoMensual, 0)
            if (fs.existsSync(`./template/Provision Ajuste VACACIONES.xlsx`)) {
                fs.unlinkSync(`./template/Provision Ajuste VACACIONES.xlsx`)
            }
            const borderStyles = {
                top: { style: "thin" },
                left: { style: "thin" },
            ...
            await wor.xlsx.writeFile(`./template/Provision Ajuste VACACIONES.xlsx`)
            const json = {
                success: true,
                message: "Exportación realizada correctamente.",
                rutaCM: "/supergen-be/template/Provision Ajuste VACACIONES.xlsx"
            }
            return json;
        } catch (error) {
    ...
    resumen: async function ({ anio, tipoPlanilla }) {
        try {
            const listaPropiedadesDatos = ["SALDO_INICIAL", "ENERO", "FEBRERO", "MARZO", "ABRIL", "MAYO", "PAGOS_MAYO", "AJUSTEPAGOS_MAYO", "JUNIO", "JULIO", "AGOSTO", "SETIEMBRE", "OCTUBRE", "NOVIEMBRE", "PAGOS_NOVIEMBRE", "AJUSTEPAGOS_NOVIEMBRE", "DICIEMBRE", "TOTAL", "ABONOS", "SALDO_ANIO"]
            const dataProcess = []
            const data = await mssqlConcar.ejecutarQueryPreparado(`exec SP_RESUMENVACACIONES '${anio}' , '${tipoPlanilla}'`, {})
    ...
    eliminarProvision: async function ({ anio, mes, tipoPlanilla, condicion }) {
        ...
        await mssqlConcar.ejecutarQueryPreparado(`delete from prov_cabecera where periodo='${periodo}' and tipoPlanilla='${tipoPlanilla}' and condicion='${condicion}' and tipoProvision='VACACIONES'`, {})
    ...
    detallesDocumentosAjustes: async function ({ periodo, tipoPlanilla, condicion }) {
        ...
        const dataComprobantes = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_AJUSTEVACACIONES '${periodo}','${tipoPlanilla}','${condicion}'`, {})
    ...
    listarAjustes: async function (detalles = []) {
        let dataProcess = []
        for (const detalle of detalles) {
            const data = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_DETALLEAJUSTEVACACIONES  '${detalle.periodo}','${detalle.tipoPlanilla}','${detalle.condicion}','${detalle.nroDiario}','${detalle.comprobante}'`, {})
    ...
    listarPago: async function ({ periodo, tipoPlanilla, condicion }) {
        ...
        const data = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_PAGOSVACACIONES  '${periodo}','${tipoPlanilla}','${condicion}'`, {})
    ...
    listarPagoDetalles: async function (detalles = []) {
        let dataProcess = []
        for (const detalle of detalles) {
            const data = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_DETALLEPAGOVACACIONES  '${detalle.periodo}','${detalle.tipoPlanilla}','${detalle.condicion}','${detalle.nroDiario}','${detalle.comprobante}'`, {})
            dataProcess = dataProcess.concat(data.map(d => ({ nroDni: d.NRO_DNI.trim(), nombres: d.NOMBRES, comprobante: detalle.comprobante, nroDiario: detalle.nroDiario, fechaCese: d.FEC_CESE, fechaIngreso: d.FEC_ING, centroCosto: d.CENTROCOSTO, area: d.AREA, montoMensual: d.MONTO_MES, pagoMensual: d.PAGO_MES, saldoMensual: d.SALO_MES, fechaPago: d.FECHA_PAGO, cuenta: d.CUENTA })))
        }
        return dataProcess
    },
    pagosDeResumen: async function ({ anio, tipoPlanilla, nroDni }) {
        const data = await mssqlConcar.ejecutarQueryPreparado(`exec SP_LEEDETALLEPAGOTRABAJADOR '${anio}','${tipoPlanilla}','VACACIONES','${nroDni}'`, {})
    ...
    exportatPagosDeResumen: async function ({ anio, tipoPlanilla, nroDni }) {
        ...
        try {
            ...
            if (fs.existsSync(`./template/Detalle_Pagos_trabajador_vacaciones.xlsx`)) {
                fs.unlinkSync(`./template/Detalle_Pagos_trabajador_vacaciones.xlsx`)
            }
            const borderStyles = {
                top: { style: "thin" },
                left: { style: "thin" },
            ...
            await wor.xlsx.writeFile(`./template/Detalle_Pagos_trabajador_vacaciones.xlsx`)
            const json = {
                success: true,
                message: "Exportación realizada correctamente.",
                rutaCM: "/supergen-be/template/Detalle_Pagos_trabajador_vacaciones.xlsx"
            }
            return json;
        } catch (error) {
            console.log("err", error)
    ...
    exportarResumido: async function ({ anio, tipoPlanilla }) {
        ...
        try {
            const rutaTemplateHC = `./template/Plantilla Resumen Provision Vacaciones Anual.xlsx`;
            if (fs.existsSync(`./template/Resumen Provision Vacaciones.xlsx`)) {
                fs.unlinkSync(`./template/Resumen Provision Vacaciones.xlsx`)
            }
            const borderStyles = {
                top: { style: "thin" },
                left: { style: "thin" },
            ...
            await wor.xlsx.writeFile(`./template/Resumen Provision Vacaciones.xlsx`)
            const json = {
                success: true,
                message: "Exportación realizada correctamente.",
                rutaCM: "/supergen-be/template/Resumen Provision Vacaciones.xlsx"
            }
            return json;
        } catch (error) {
            console.log("err", error)
    ...
    listarDetalles: async function (detalles) {
        let dataProcess = []
        for (const detalle of detalles) {
            const data = await mssqlConcar.ejecutarQueryPreparado(`EXEC SP_LEEDIARIO_DETALLEVACACIONES  '${detalle.periodo}','${detalle.tipoPlanilla}','${detalle.condicion}','${detalle.nroDiario}','${detalle.comprobante}'`, {})
            if (data) {
    ...
    detalleAjusteTrabajador: async function ({ anio, tipoPlanilla, nroDni }) {
        const data = await mssqlConcar.ejecutarQueryPreparado(`exec SP_LEEDETALLEAJUSTETRABAJADOR '${anio}','${tipoPlanilla}','VACACIONES','${nroDni}'`, {})
...
module.exports = provisionVacaciones
