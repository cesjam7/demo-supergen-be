...
const proyStandardMacho = {

    guardar: async function (proyStandardHembra, userIdRegistro) {
        const connection = await mysql.connection();
        try {
            await connection.query("START TRANSACTION");
            await connection.query("insert into proy_standard_macho(semana,idprod,porc_postura,porc_hi,porc_nacimiento,fec_reg,usuario_reg) values(?,?,?,?,?,?,?)", [proyStandardHembra.semana, 1,
            proyStandardHembra.porc_postura, proyStandardHembra.porc_hi, proyStandardHembra.porc_nacimiento, new Date(), userIdRegistro])
    ...
    importar: async function (excelRuta) {
        ...
        try {
            ...
            const dataBd = await connection.query("select * from proy_standard_macho where estado<>0 order by semana")
            ...
            const queryMapUpdate = dataEncontradaA.map(s => `update proy_standard_macho set porc_postura=${s.porc_postura},porc_hi=${s.porc_hi},porc_nacimiento=${s.porc_nacimiento} where semana=${s.semana}`)
    ...
    exportarExcel: async function ({ data = [] }) {
        ...
        try {
            const rutaTemplateHC = `./template/Plantilla Exportacion Machos.xlsx`;
            ...
            if (fs.existsSync(`./template/proyeccion machos.xlsx`)) {
                fs.unlinkSync(`./template/proyeccion machos.xlsx`)
            }
            ...
            const json = {
                ...
                rutaCM: "/supergen-be/template/proyeccion machos.xlsx"
            }
            await workbook.xlsx.writeFile(`./template/proyeccion machos.xlsx`)
    ...
    eliminar: async function (proyStandardMachoId) {
        const connection = await mysql.connection();
        try {
            await connection.query("START TRANSACTION");
            await connection.query("update proy_standard_macho set estado=0 where idStandardProyMacho=?", [proyStandardMachoId])
            await connection.query("COMMIT")
        }
        catch (error) {
            throw error;
    ...
    editar: async function (proyStandardHembra) {
        ...
        try {
            await connection.query("START TRANSACTION");
            await connection.query("update proy_standard_macho set semana=?,porc_postura=?,porc_hi=?,porc_nacimiento=? where idStandardProyMacho=?", [proyStandardHembra.semana,
            proyStandardHembra.porc_postura, proyStandardHembra.porc_hi, proyStandardHembra.porc_nacimiento, proyStandardHembra.idStandardProyMacho])
            await connection.query("COMMIT")
        }
        catch (error) {
            throw error;
    ...
    listar: async function () {
        const connection = await mysql.connection();
        try {
            return await connection.query("select * from proy_standard_macho where estado<>0 order by semana")
        }
        catch (error) {
            throw error;
        } finally {
...
module.exports = proyStandardMacho;